...
public static class GUILayoutHelper
{
	...
	static public bool IsUniform<T> (GameObject go) where T : MonoBehaviour
	{
		...
		if (go.GetComponent<T>() != null)
		{
			...
			return parent == null || IsUniform<T>(parent.gameObject);
		}
	...
	static public void FixUniform<T> (GameObject go) where T : MonoBehaviour
    {
		Transform t = go.transform;

		while (t != null && t.gameObject.GetComponent<T>() == null)
		{
			if (!IsUniform(t.localScale))
			{
				RegisterUndo("Uniform scaling fix", t);
	...
	static public bool DrawMinimalisticHeader (string text) { return DrawHeader(text, text, false, false); }
	...
	static public SerializedProperty DrawProperty (string label, SerializedObject serializedObject, string property, bool padding, params GUILayoutOption[] options)
	{
		...
		if (sp != null)
		{
//			if (NGUISettings.minimalisticLook)   //liangzg
                padding = false;
